...
ARCGIS_PRO_PATH_EXPORT <- 'C:/Users/Patricia/Documents/ArcGIS/Projects/MyProject/Exports/dades_enriquides'
...
```{r message=FALSE}
DATA_PATH_INFRAESTR <- file.path(DATA_PATH, 'infraestructura_ciclable')
...
```{r message=FALSE}
DATA_PATH_US <- file.path(DATA_PATH, 'aforament')
...
```{r message=FALSE}
DATA_PATH_ACC <- file.path(DATA_PATH, 'accidents')
...
```{r message=FALSE}
DATA_PATH_US <- file.path(DATA_PATH, 'qualitat_aire')
...
```{r}
write.csv(statsWAB, 'estadistiques.csv', row.names = FALSE)
```
...
dmTransit <- load_data(ARCGIS_PRO_PATH_EXPORT, 'districtes_mesures_transit', 'zip', rename=FALSE)
dmBici <- load_data(ARCGIS_PRO_PATH_EXPORT, 'districtes_mesures_bicis', 'zip', rename=FALSE)
...
```{r}
write.csv(dm, 'estadistiquesDistrictes.csv', row.names = FALSE)
```
...
```{r}
#carregar les dades tractades a ArcGIS Pro
bmTransit <- load_data(ARCGIS_PRO_PATH_EXPORT, 'barris_mesures_transit', 'zip', rename=FALSE)
bmBici <- load_data(ARCGIS_PRO_PATH_EXPORT, 'barris_mesures_bicis', 'zip', rename=FALSE)
...
```{r}
write.csv(bm, 'estadistiquesBarris.csv', row.names = FALSE)
```
...
```{r}
write.csv(df_mMensual, 'estadistiquesMensuals.csv', row.names = FALSE)
```
...
animate(g, end_pause = 15, renderer=gifski_renderer("usDistrictes.gif"))
```
...
animate(g, height=800, width=600, end_pause = 15, renderer=gifski_renderer("usBarris.gif"))
```
...
```{r message=FALSE, warning = FALSE}
st_write(sfAccidentats, dsn = "accidentats.shp", driver = "ESRI Shapefile")
```
...
for (e in unique(dfEstacions$stationId)){
  ...
  dfStats <- aggregate(XVPCA_e$ICQA, by=list(stationId=XVPCA_e$CODI.EOI, pollutant=XVPCA_e$CONTAMINANT, year=XVPCA_e$year, month=XVPCA_e$month), FUN=mean)
  dfStats <- merge(dfStats, dfEstacions, by=c('stationId'))
  dfStats$Temporality <- as.yearmon(paste0(dfStats$year, '-', dfStats$month))
  ...
  animate(g, end_pause = 15, width = 600, height = 300, renderer=gifski_renderer(paste0(e,'.gif')))
}
```
