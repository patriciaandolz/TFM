...
```{r}
write.csv(statsWAB, paste0(AGOL_PATH, '/', 'estadistInfraestr.csv'), row.names = FALSE)
```
...
```{r}
write.csv(dm, paste0(AGOL_PATH, '/', 'estadistUsDistrictes.csv'), row.names = FALSE)
```
...
```{r}
write.csv(bm, paste0(AGOL_PATH, '/', 'estadistUsBarris.csv'), row.names = FALSE)
```
...
```{r}
write.csv(df_mMensual, paste0(AGOL_PATH, '/', 'estadistUsMensuals.csv'), row.names = FALSE)
```
...
animate(g, end_pause = 15, renderer=gifski_renderer(paste0(AGOL_PATH, '/', "usDistrictes.gif")))
```
...
animate(g, height=800, width=600, end_pause = 15, 
        renderer=gifski_renderer(paste0(AGOL_PATH, '/', "usBarris.gif")))
```
...
```{r message=FALSE, warning = FALSE}
st_write(sfAccidentats, dsn = paste0(AGOL_PATH, '/', "accidentats.shp"), driver = "ESRI Shapefile")
```
...
for (e in unique(dfEstacions$stationId)){
  ...
  animate(g, end_pause = 15, width = 600, height = 300, 
          renderer=gifski_renderer(paste0(AGOL_PATH, '/', 'estacioQA_', e,'.gif')))
}
```
